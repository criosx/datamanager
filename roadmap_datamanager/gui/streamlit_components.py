...
def UI_fragment_SSH_connection(cfg):
    ...
    config_file = dgapi.ssh_config_path()
    found, message = dgapi.ssh_config_has_entry(ssh_host_alias, ssh_hostname, ssh_host_user)

    suggested_private_key = dgapi.ssh_default_key_path(ssh_hostname, ssh_host_user)
    ...
    with stc2:
        if st.button("Test SSH Connection", type='primary'):
            ok, summary, details = dgapi.ssh_test_connection(ssh_host_alias)
            if ok:
                st.success(summary)
            else:
                st.error(summary)
